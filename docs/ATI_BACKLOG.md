...
- 2026-08-26 - FOREX-FIRST pivot per operator. TRADING_MODE=forex default: MEXC poller + CCXT loop gated off; MT5 adapter is the primary feed across the 29-instrument fx universe (majors/crosses/XAU) with canonical payload symbols + configurable broker prefix (frx for Deriv); currencies ride in every payload so the macro veto maps each fx leg automatically. Dashboard boot-branches on /v1/operator/system: TradingView free widget as chart surface (legitimate embed; execution prices stay broker-grade MT5 -- TradingView has no legal public data API, operator request honored as display-only), 1.5s tick poll with mid/flash/spread top-of-book panel, session clock chip, fx pair search, tape hidden. 3 new tests; 1749 green; CI green (8a31d63). Learning status restated honestly: record->reflect->recall is live (episodes into Omega prompts); parameter self-modification stays constitutionally locked until evidence gates pass. NEXT: enable MT5 terminal + FF_ENABLED=true in .env for overnight accumulation of fx reaction library; then fx-specific ablation (surprise features vs rules-only) and spread-aware fill model in PaperFillEngine (fx spreads are the cost king).
