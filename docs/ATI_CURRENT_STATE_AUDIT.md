...
The paper fill engine does not model venue bookkeeping; the sandbox venue
(`backend/application/simulation/sandbox_venue.py`, item #27) now owns the
realistic order lifecycle on the production-equivalent path:

- post-only orders rest and later fill (guarded `VenueOrderState` transitions);
- queue position is modeled (FIFO price-time queue with `queue_position`);
- partial fills are modeled (depth-aware VWAP, `remaining_quantity`);
- cancellations are a complete lifecycle (resting-only, terminal);
- fees are configurable in paper fills (default zero, taker/maker on notional);
- funding is modeled deterministically when configured (default unmodeled);
- slippage is attributable (arrival-price based);
- deterministic TTL expiry replaces an open-ended resting life;
- rejection semantics are explicit (post-only / FOK);
- the venue self-reports positions for reconciliation (`VenueStateSource`).
...
Only after evidence (status 2026-08-13):

1. ✅ Paper autonomy (P4-004: long autonomous paper campaign harness — built).
2. ✅ Long paper campaign (P4-004: unattended paper campaign runner — built).
3. ✅ Canary live deployment (P4-003: canary harness, operator-authorized — built; live start requires operator wiring + credentials).
4. ⏳ Small capital (operational act, not code).
5. ✅ automated safe-mode (supervisor service, circuit breakers, auto-rollback P4-001 — built).
6. ✅ rollback (P4-001 automatic stay-limit demotion — built).
7. ✅ gradual scaling (P4-005: post-canary progressive capital ramp harness — built).
8. ✅ production model promotion (P4-001 controlled promotion ladder — built).
9. ✅ composed autonomy program (P4-006: one deterministic candidate run across the whole research→production ladder, stages injected and recorded — built).
...
| Controlled autonomy machinery | 75% |
